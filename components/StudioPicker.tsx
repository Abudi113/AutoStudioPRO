      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-14 flex-1 overflow-y-auto pr-1 pb-10">
        {STUDIO_PRESETS.map((studio) => (
          <div
            ...
            className={`group cursor-pointer rounded-[32px] overflow-hidden border-2 transition-all relative aspect-video shadow-xl ${selectedStudio.id === studio.id
          ...
          >
            ...
            <div className="absolute top-5 left-5 md:top-8 md:left-8">
              <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest backdrop-blur-xl border border-white/10 ${studio.category === 'Premium' ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white'
                }`}>
            ...
            <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-10 pointer-events-none">
              <h4 className="text-xl md:text-3xl font-black mb-2 text-white tracking-tight leading-tight">{t[studio.name] || studio.name}</h4>
              <p className="text-gray-300 text-xs md:text-base font-medium line-clamp-1 opacity-90">{t[studio.description] || studio.description}</p>
            </div>

            {selectedStudio.id === studio.id && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/10 backdrop-blur-[2px]">
                <div className={`w-16 h-16 md:w-24 md:h-24 rounded-full bg-blue-600 flex items-center justify-center shadow-2xl animate-in zoom-in-50 duration-300 border-4 border-white/20`}>
